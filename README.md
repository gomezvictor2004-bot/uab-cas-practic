...
Asistente Universitario Inteligente que centraliza la información académica y administrativa para estudiantes. Este repositorio contiene el backend en Node.js con un servidor HTTP ligero (sin dependencias externas por defecto) y la definición de los servicios clave: scraping académico con Puppeteer, automatizaciones con n8n, resúmenes con IA y notificaciones multi-canal.
...
- **Servidor HTTP en Node.js** expone una API REST con endpoints para tareas, documentos, briefing diario y estado del sistema.
...
├── public/
│   ├── index.html             # Panel web servido por el servidor HTTP ligero
│   ├── main.js                # Lógica de cliente para consumir la API
...
│   ├── controllers/            # Lógica HTTP (validaciones y respuestas)
│   ├── services/               # Casos de uso (scraping, IA, cron…)
│   ├── utils/                  # Utilidades comunes (logger, errores)
│   ├── server.js               # Servidor HTTP ligero (API + estáticos)
...
```bash
# Instalar dependencias (no descarga paquetes: el servidor funciona solo con Node.js)
npm install
...
> **Nota:** con `MOCK_SCRAPER=true` el backend devuelve datos de ejemplo sin lanzar Chrome. Para producción, instala `puppeteer` (`npm install puppeteer`), establece `MOCK_SCRAPER=false` y ajusta los selectores en `scrapingService`.

### Dependencias opcionales

- `openai`: instala `npm install openai` para habilitar los resúmenes con el SDK oficial (requiere `OPENAI_API_KEY`).
- `puppeteer`: instala `npm install puppeteer` para activar el scraping real cuando desactives `MOCK_SCRAPER`.
- Puedes añadir otras librerías (axios, node-cron, etc.) según tus integraciones; la demo funciona únicamente con las APIs nativas de Node.js.
...
## 🖥️ Interfaz web incluida

La carpeta `public/` contiene un dashboard estático que se sirve automáticamente desde el propio servidor HTTP. Al arrancar el proyecto (`npm run dev`), visita `http://localhost:3000` para:

- Ver el estado del servicio con indicadores visuales.
- Consultar el briefing diario y las tareas mock que devuelve la API.
- Enviar apuntes al endpoint `/api/documents/summary` mediante el formulario y obtener el resumen directamente en pantalla.

La interfaz consume los mismos endpoints REST, por lo que cualquier despliegue (Render, Railway, etc.) expondrá la API y la web desde la misma URL.
...
```
Planificador diario interno → syncAcademicData → dispatchDailyBriefings → n8n webhook → canales destino
```
...
5. Extender el panel web con dashboards personalizados y autenticación de usuarios finales.
